...
int minimax(unsigned long long int & r,unsigned long long int & n,unsigned long long int & b,
        ...
        unsigned long long int & forbcstlS, unsigned long long int & forbcstlL, std::vector<int>& rootVariant,
    ...
    if(it != ump.end()){
        ...
        if (maximizingPlayer){
            ...
            for (int semIdx = 0 ; semIdx < moves.size()/2 ; semIdx++){
                ...
                eval = minimax(r,n,b,q,k,p,R,N,B,Q,K,P,CstlL,CstlS,cstlL,cstlS,enPassant,tempForbCstlS,tempForbCstlL,tempForbcstlS,tempForbcstlL, tempSuffix, depthInit-1,depth-1, veryMaxDepth-1, alpha,beta,false,positionHash) ;
                ump[positionHash] = {eval,veryMaxDepth} ;
            ...
            for (int semIdx = 0 ; semIdx < moves.size()/2 ; semIdx++){
                ...
                eval = minimax(r,n,b,q,k,p,R,N,B,Q,K,P,CstlL,CstlS,cstlL,cstlS,enPassant,tempForbCstlS,tempForbCstlL,tempForbcstlS,tempForbcstlL, tempSuffix, depthInit-1,depth-1,veryMaxDepth-1,alpha,beta,true,positionHash) ;
                ump[positionHash] = {eval,veryMaxDepth} ;
