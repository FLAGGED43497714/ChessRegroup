...
void Do_move(int pos, int dest, bool & CstlL, bool & CstlS, bool & cstlL, bool & cstlS,
    ...
    unsigned long long int const CSHORTHASH = 1615461785985833611 ;
    unsigned long long int const CLONGHASH = 803003761016756798 ;
    ...
    if (r>>pos&1){
        if(pos == 56){
            brokecastleS = cstlS ;
            if(brokecastleS){positionHash ^= cSHORTHASH ;}
            cstlS = false ;
        }
        if(pos == 63){
            brokecastleL = cstlL ;
            if(brokecastleL){positionHash ^= cLONGHASH ;}
            cstlL = false ;
        }
        r -= posInt ;
        r += destInt ;
...
void Undo_move(int wasPos, int wasDest, bool brokeCastleS, bool brokeCastleL, bool brokecastleS, bool brokecastleL,
    ...
    positionHash^= 891517893554500602 ;
